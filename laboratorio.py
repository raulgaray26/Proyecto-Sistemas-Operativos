...
class Laboratorio:
    ...
    def abrir(self):
        ...
        # Inicio escalonado: pequeña pausa entre hilo y hilo
        for hilo in self.cola_espera:
            hilo.start()
            time.sleep(DELAY_ENTRE_HILOS)
    ...
    def usar_computadora(self, id_estudiante, nombre, tarea, tiempo_uso):
        print(f"[{id_estudiante:02d}] {nombre} está esperando una computadora...")
        ...
        try:
            ...
            print(f"[{id_estudiante:02d}] {nombre} → PC {numero_pc} | Tarea: '{tarea}' | Tiempo: {tiempo_uso}s | Disponibles: {self.computadoras_disponibles}/{self.capacidad}")
            ...
            print(f"[{id_estudiante:02d}] {nombre} terminó '{tarea}' en PC {numero_pc} | Disponibles: {self.computadoras_disponibles}/{self.capacidad}")
